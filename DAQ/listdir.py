...
def doMerging():
   ...
   while 1:
	...
	if(fileNameString[5] == "std"):
	   ...
	   inputFile1 = inputFile0.replace(".std.",".mon.")
	   inputFile2 = inputFile0.replace(".std.",".met.")
	   ...
	   """
	      ...
	      if(debug >= 0): print "Time for merging: ",endMergingTime-initMergingTime

              file_met = open(inputFile2, 'r')
              line=file_met.readline()
	      file_met.close()
	      lineStrip = line.strip()
	      lineStripString = lineStrip.split(' ')

	      outFile2 = "%s/%s.%s.%s.%s.%s.dat" % (path_to_merge,fileNameString[0],fileNameString[1],fileNameString[2],fileNameString[3],"met")
              if os.path.exists(outFile2):
        	 file_metOutput = open(outFile2, 'r')
        	 line=file_metOutput.readline()
		 file_metOutput.close()
		 lineStripOutput = line.strip()
		 lineStripStringOutput = lineStripOutput.split(' ')
	      else:
		 lineStripStringOutput = [0, 0]

	      if(len(lineStripString) >= 2 and len(lineStripStringOutput) >= 2):
		 ...
		 msg = "wrong format: %s / %s" % (lineStripString,lineStripStringOutput)
        	 raise RuntimeError, msg

	      msg = "echo %f %f > %s" % (lineStripStringOutput[0],lineStripStringOutput[1],outFile2)
	      if(debug > 0): print msg
	      os.system(msg)
	      os.remove(inputFile2)
	elif(fileNameString[5] == "eol"):
           file_eol = open(os.path.join(path_to_watch, afterString[i]), 'r')
           line=file_eol.readline()
	   file_eol.close()
	   lineStrip = line.strip()
	   lineStripString = lineStrip.split(' ')

	   outFile0 = "%s/%s.%s.%s.%s.%s.dat" % (path_to_merge,fileNameString[0],fileNameString[1],fileNameString[2],fileNameString[3],"met")
           if os.path.exists(outFile0):
              file_metOutput = open(outFile0, 'r')
              line=file_metOutput.readline()
	      file_metOutput.close()
	      lineStripOutput = line.strip()
	      lineStripStringOutput = lineStripOutput.split(' ')
