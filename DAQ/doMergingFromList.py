...
# expected location for unmerged files
# /data/run<X>/hlt/<host>_DATA 
#
# expected location for monitoring files
# /data/run<X>/hlt/<host>_MON
#
# another folder is needed:
...
def mergeFiles(outMergedFile, inputDataFolder, files, inputJsonFile):
   ...
   msg  = "rm -f %s; touch %s;" % (outMergedFile,outMergedFile)
...
def doTheMerging():
   while 1:
	  ...
	  processs = []
	     ...
	     # merged file
	     outMergedFile = settings['outputName']
