...
void optimalCuts_42x_LP
(
 int     mH  	 = 29,
 Char_t xTitle[]="myX", Char_t yTitle[]="Fraction",
 int thePlot = 0,
 TString bgdInputFile    = "ntuples_42x_LP/background42x_met20.root",
 TString signalInputFile = "ntuples_42x_LP/hww160.root",
 TString dataInputFile   = "ntuples_42x_LP/data_2l_met20.root",
 ...
 )
{
  	...
  	if((channel == 29 || (channel >= 100 && channel <= 800) || channel == 900 || channel == 901 || channel == 5) &&
	   channel != 128){
  	  ...
  	  if((fDecay == 5 ||fDecay == 13)) {
  	    if(bgdEvent.njets_ == 0) add=add*1.51;
  ...
  if((channel >= 0 && channel <= 8000)){
    ...
    if(passCuts == true){
      double add = 1.;
      add = add*nVtxScaleFactor(fhDNvtx,sigEvent.nvtx_);
	...
	//add = add * enhancementFactor(channel);
      ...
      if(TMath::Abs((sigEvent.sfWeightFR_*sigEvent.sfWeightPU_*sigEvent.sfWeightEff_*sigEvent.sfWeightTrig_*sigEvent.sfWeightHPt_)-add)/add>0.0001) {
        ...
        printf("                    %f %f %f %f %f\n",1.0,nVtxScaleFactor(fhDNvtx,sigEvent.nvtx_),leptonEfficiency(sigEvent.lep1_.Pt(), sigEvent.lep1_.Eta(), fhDEffMu, fhDEffEl, sigEvent.lid1_)*
  ...
  double bgdCombinedE[4] = {sqrt(weiDecay[0]+weiDecay[1]+weiDecay[2]+weiDecay[3]),
                            sqrt(weiDecay[6]+weiDecay[7]+weiDecay[8]+weiDecay[10]+weiDecay[31]),
