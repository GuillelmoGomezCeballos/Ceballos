...
#include "/home/ceballos/releases/CMSSW_5_2_3_patch3/src/Ana/nt_scripts/makeSystematicEffects.h"
#include <TROOT.h>
#include <TFile.h>
#include <TTree.h>
#include <iostream>
 ...
 )
{
  ...
  if     (thePlot >=  8 && thePlot <=  8) {nBinPlot = 100; xminPlot = 200.0; xmaxPlot = 700.0;}
  else if(thePlot >= 13 && thePlot <= 13) {nBinPlot = 100; xminPlot =   0.0; xmaxPlot = 400.0;}
  ...
  else if(thePlot >= 20 && thePlot <= 23) {nBinPlot = 180; xminPlot = 0.0; xmaxPlot = 180.0;}
  ...
  const int nBinMVA = 9;
  Float_t xbins[nBinMVA+1] = {200, 250, 300, 375, 450, 525, 600, 700, 800, 900};
  ...
  for (int evt=0; evt<nBgd; ++evt) {
    ...
    makeSystematicEffects(bgdEvent.lid1_, bgdEvent.lid2_, bgdEvent.lep1_, bgdEvent.lep2_, bgdEvent.dilep_, 
			 ...
			 outputVar);
    double MVAVar[6] = {outputVar[8],outputVar[8],outputVar[8],outputVarLepP[8],outputVarLepM[8],outputVarMET[8]};
    ...
    if(bgdEvent.jet1_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(bgdEvent.jet2_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(bgdEvent.jet3_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(bgdEvent.jet4_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(bgdEvent.jet1_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;
    if(bgdEvent.jet2_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;
    if(bgdEvent.jet3_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;
    if(bgdEvent.jet4_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;

    if(channel > 1000 && channel < 2000){ // ZH->2l+inv selection
      if(charge == 0 && NjetSyst[0] == 0               && outputVar[4]     > 50.0 && outputVar[0]     > 20.0 &&  outputVar[1]	  > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVar[2]-91.1876)	 < 15. && passDPhiLL && outputVar[4]	 > cutValue[0] && outputVar[9]*180.0/TMath::Pi()     > cutValue[1] && outputVar[11]	< cutValue[2]) passSystCuts[lType][JESUP] = true;
      if(charge == 0 && NjetSyst[1] == 0               && outputVar[4]     > 50.0 && outputVar[0]     > 20.0 &&  outputVar[1]	  > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVar[2]-91.1876)	 < 15. && passDPhiLL && outputVar[4]	 > cutValue[0] && outputVar[9]*180.0/TMath::Pi()     > cutValue[1] && outputVar[11]	< cutValue[2]) passSystCuts[lType][JESDOWN] = true;
      if(charge == 0 && bgdEvent.jet1_.Pt() < ptJetMax && outputVarLepP[4] > 50.0 && outputVarLepP[0] > 20.0 &&  outputVarLepP[1] > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVarLepP[2]-91.1876) < 15. && passDPhiLL && outputVarLepP[4] > cutValue[0] && outputVarLepP[9]*180.0/TMath::Pi() > cutValue[1] && outputVarLepP[11] < cutValue[2]) passSystCuts[lType][LEPP] = true;
      if(charge == 0 && bgdEvent.jet1_.Pt() < ptJetMax && outputVarLepM[4] > 50.0 && outputVarLepM[0] > 20.0 &&  outputVarLepM[1] > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVarLepM[2]-91.1876) < 15. && passDPhiLL && outputVarLepM[4] > cutValue[0] && outputVarLepM[9]*180.0/TMath::Pi() > cutValue[1] && outputVarLepM[11] < cutValue[2]) passSystCuts[lType][LEPM] = true;
      if(charge == 0 && bgdEvent.jet1_.Pt() < ptJetMax && outputVarMET[4]  > 50.0 && outputVarMET[0]  > 20.0 &&  outputVarMET[1]  > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVarMET[2]-91.1876)  < 15. && passDPhiLL && outputVarMET[4]  > cutValue[0] && outputVarMET[9]*180.0/TMath::Pi()  > cutValue[1] && outputVarMET[11]  < cutValue[2]) passSystCuts[lType][MET] = true;
      if(
         charge == 0 &&
	 bgdEvent.jet1_.Pt() < ptJetMax &&
	 theMET > 50.0 &&
	...
	theWeight              = bgdEvent.scale1fb_*lumi*add;
      ...
      if(passCuts[1][SIGSEL] && theMET > 50.0){ // begin making plots
  ...
  for (int evt=0; evt<nSyst; ++evt) {
	...
	if(isRealLepton == false &&
	 ...
	 //systEvent.jet1_.Pt() < ptJetMax &&
	 theMET > 50.0 ) {
	 if( passBtagVeto && passMET && passDPhiZMET && passPTFrac) passCuts[0][SIGSEL]  = true;
      }
    ...
    if     (fDecay == 1 && passCuts[0][SIGSEL]){
      ...
      makeSystematicEffects(systEvent.lid1_, systEvent.lid2_, systEvent.lep1_, systEvent.lep2_, systEvent.dilep_, 
			    ...
			    outputVar);
      double MVAVar[6] = {outputVar[8],0,0,0,0,0};
      for(int nv=0; nv<6; nv++) MVAVar[nv] = TMath::Min(TMath::Max(MVAVar[nv],xbins[0]+0.001),xbins[nBinMVA]-0.001);
  ...
  for (int evt=0; evt<nSig; ++evt) {
    ...
    double NjetSyst[2] = {0., 0.};
    if(sigEvent.jet1_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(sigEvent.jet2_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(sigEvent.jet3_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(sigEvent.jet4_.Pt()*1.05 > ptJetMax) NjetSyst[0]++;
    if(sigEvent.jet1_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;
    if(sigEvent.jet2_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;
    if(sigEvent.jet3_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;
    if(sigEvent.jet4_.Pt()*0.95 > ptJetMax) NjetSyst[1]++;

    if(channel > 1000 && channel < 2000){ // ZH->2l+inv selection
      if(charge == 0 && NjetSyst[0] == 0               && outputVar[4]     > 50.0 && passDPhiLL && outputVar[0]      > 20.0 &&  outputVar[1]    > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVar[2]-91.1876)     < 15. && outputVar[4]     > cutValue[0] && outputVar[9]*180.0/TMath::Pi()     > cutValue[1] && outputVar[11]     < cutValue[2]) passSystCuts[lType][JESUP] = true;
      if(charge == 0 && NjetSyst[1] == 0               && outputVar[4]     > 50.0 && passDPhiLL && outputVar[0]      > 20.0 &&  outputVar[1]    > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVar[2]-91.1876)     < 15. && outputVar[4]     > cutValue[0] && outputVar[9]*180.0/TMath::Pi()     > cutValue[1] && outputVar[11]     < cutValue[2]) passSystCuts[lType][JESDOWN] = true;
      if(charge == 0 && sigEvent.jet1_.Pt() < ptJetMax && outputVarLepP[4] > 50.0 && passDPhiLL && outputVarLepP[0] > 20.0 &&  outputVarLepP[1] > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVarLepP[2]-91.1876) < 15. && outputVarLepP[4] > cutValue[0] && outputVarLepP[9]*180.0/TMath::Pi() > cutValue[1] && outputVarLepP[11] < cutValue[2]) passSystCuts[lType][LEPP] = true;
      if(charge == 0 && sigEvent.jet1_.Pt() < ptJetMax && outputVarLepM[4] > 50.0 && passDPhiLL && outputVarLepM[0] > 20.0 &&  outputVarLepM[1] > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVarLepM[2]-91.1876) < 15. && outputVarLepM[4] > cutValue[0] && outputVarLepM[9]*180.0/TMath::Pi() > cutValue[1] && outputVarLepM[11] < cutValue[2]) passSystCuts[lType][LEPM] = true;
      if(charge == 0 && sigEvent.jet1_.Pt() < ptJetMax && outputVarMET[4]  > 50.0 && passDPhiLL && outputVarMET[0]  > 20.0 &&  outputVarMET[1]  > 20.0 && passBtagVeto && pass3rLVeto && fabs(outputVarMET[2]-91.1876)  < 15. && outputVarMET[4]  > cutValue[0] && outputVarMET[9]*180.0/TMath::Pi()  > cutValue[1] && outputVarMET[11]  < cutValue[2]) passSystCuts[lType][MET] = true;
      if(
         charge == 0 &&
	 sigEvent.jet1_.Pt() < ptJetMax &&
	 theMET > 50.0 &&
  ...
  outFilePlotsNote->cd();
    ...
    double scaleZ = (histo5->GetSumOfWeights()-(histo1->GetSumOfWeights()+histo2->GetSumOfWeights()+histo3->GetSumOfWeights()+histo4->GetSumOfWeights()))/histo0->GetSumOfWeights();
  ...
  if(nSelectedData[SIGSEL] <= 0) NFinal[4] = (1.0-EMbkg)*NemFact;
  ...
  double ZLLMCUnc = sqrt(bgdCombinedE[BCK1REG+nSelTypes][0] /bgdCombined[BCK1REG+nSelTypes][0] /bgdCombined[BCK1REG+nSelTypes][0]+
                  	 bgdCombinedE[BCK2REG+nSelTypes][0] /bgdCombined[BCK2REG+nSelTypes][0] /bgdCombined[BCK2REG+nSelTypes][0]+
                  	 bgdCombinedE[BCK12REG+nSelTypes][0]/bgdCombined[BCK12REG+nSelTypes][0]/bgdCombined[BCK12REG+nSelTypes][0]);
                    
  printf("ZLL MC BCK1REG*BCK2REG/BCK12REG=%f*%f/%f = %8.3f +/- %5.3f --> %8.3f +/- %5.3f\n",
         bgdCombined[BCK1REG+nSelTypes][0],bgdCombined[BCK2REG+nSelTypes][0],bgdCombined[BCK12REG+nSelTypes][0],
	 bgdCombined[BCK1REG+nSelTypes][0]*bgdCombined[BCK2REG+nSelTypes][0]/bgdCombined[BCK12REG+nSelTypes][0],
	 bgdCombined[BCK1REG+nSelTypes][0]*bgdCombined[BCK2REG+nSelTypes][0]/bgdCombined[BCK12REG+nSelTypes][0]*ZLLMCUnc,
	 bgdCombined[SIGSEL+nSelTypes][0],sqrt(bgdCombinedE[SIGSEL+nSelTypes][0]));

  double ZdataBCK1REG  = nSelectedData[BCK1REG+nSelTypes] -bgdCombined[BCK1REG+nSelTypes][1] -bgdCombined[BCK1REG+nSelTypes][2]-
                                                           bgdCombined[BCK1REG+nSelTypes][3] -bgdCombined[BCK1REG+nSelTypes][4];
  ...
  double ZdataBCK1REGE = sqrt(nSelectedData[BCK1REG+nSelTypes]
         +bgdCombinedE[BCK1REG+nSelTypes][1]+bgdCombinedE[BCK1REG+nSelTypes][2]
         +bgdCombinedE[BCK1REG+nSelTypes][3]+bgdCombinedE[BCK1REG+nSelTypes][4]);
  double ZdataBCK2REGE = sqrt(nSelectedData[BCK2REG+nSelTypes]
  ...
  double ZdataBCK12REGE = sqrt(nSelectedData[BCK12REG+nSelTypes]
  ...
  double ZdataBCK = ZdataBCK1REG*ZdataBCK2REG/ZdataBCK12REG;
  double ZdataBCKE = sqrt(ZdataBCK1REGE *ZdataBCK1REGE /ZdataBCK1REG /ZdataBCK1REG+
                          ZdataBCK2REGE *ZdataBCK2REGE /ZdataBCK2REG /ZdataBCK2REG+
			  ZdataBCK12REGE*ZdataBCK12REGE/ZdataBCK12REG/ZdataBCK12REG);

  NFinal[0] = bgdCombined[SIGSEL+nSelTypes][0]; NFinalE[0] = 2.0;
  printf("ZLL DA =(%f +/- %f)*(%f +/- %f)/(%f +/- %f) = (%f +/- %f)\n",
         ZdataBCK1REG ,ZdataBCK1REGE,
         ZdataBCK2REG ,ZdataBCK2REGE,
         ZdataBCK12REG,ZdataBCK12REGE,
  ...
  //if(var0 == 120 && var1 == 160 && var2 == 0.2)
    sprintf(outputLimitsCut,"histo_limits_zhinv%2s_mh%3d_cut_%4s.txt",finalStateName,mH,ECMsb.Data());
  //else
  //  sprintf(outputLimitsCut,"histo_limits_zhinv%2s_mh%3d_cut_%4s_%.0f_%.0f_%.0f.txt",finalStateName,mH,ECMsb.Data(),var0,var1,var2*100);
  ofstream newcardCut;
  ...
  newcardCut << Form("pdf_qqbar 		  lnN %5.3f   -     -   1.050 1.060   -     -  \n",1.069);
  ...
  //double nZjets = histo_Zjets->GetSumOfWeights();
  if(histo_GAMMA_JETS->GetSumOfWeights() > 0 && histo_Zjets->GetSumOfWeights() > 0) {
    printf("===> g+jets vs. z+jets: %f vs. %f\n",histo_GAMMA_JETS->GetSumOfWeights(),histo_Zjets->GetSumOfWeights());
    histo_GAMMA_JETS->Scale(histo_Zjets->GetSumOfWeights()/histo_GAMMA_JETS->GetSumOfWeights());
  }
  for(int i=1; i<=histo_ZH->GetNbinsX(); i++){
    //histo_Zjets->SetBinContent(i,nZjets/histo_ZH->GetNbinsX());
    //histo_Zjets->SetBinError  (i,nZjets/histo_ZH->GetNbinsX());
    if(histo_GAMMA_JETS->GetSumOfWeights() > 0 && histo_Zjets->GetSumOfWeights()){
  ...
  sprintf(outputLimits,"zhinv%2s_%3d.input_%4s.root",finalStateName,mH,ECMsb.Data());
  ...
  //if(var0 == 120 && var1 == 160 && var2 == 0.2)
    sprintf(outputLimitsShape,"histo_limits_zhinv%2s_mh%3d_shape_%4s.txt",finalStateName,mH,ECMsb.Data());
  //else
  //  sprintf(outputLimitsShape,"histo_limits_zhinv%2s_mh%3d_shape_%4s_%.0f_%.0f_%.0f.txt",finalStateName,mH,ECMsb.Data(),var0,var1,var2*100);
  ...
  newcardShape << Form("pdf_qqbar 		               lnN  %5.3f   -     -   1.050 1.060   -     -  \n",1.069);
  ...
  newcardShape << Form("CMS_zhinv%s_MVAZHStatBounding_%s     shape  1.000   -     -     -     -     -     -  \n",finalStateName,ECMsb.Data());
  if(NFinal[1] > 0)
  newcardShape << Form("CMS_zhinv%s_MVAVVVStatBounding_%s    shape    -     -	1.000   -     -     -     -  \n",finalStateName,ECMsb.Data());
  newcardShape << Form("CMS_zhinv%s_MVAWZStatBounding_%s     shape    -     -     -   1.000   -     -     -  \n",finalStateName,ECMsb.Data());
  newcardShape << Form("CMS_zhinv%s_MVAZZStatBounding_%s     shape    -     -     -     -   1.000   -     -  \n",finalStateName,ECMsb.Data());
  if(NFinal[4] > 0)
  newcardShape << Form("CMS_zhinv%s_MVAEMStatBounding_%s     shape    -     -	  -     -     -   1.000   -  \n",finalStateName,ECMsb.Data());
  if(NFinal[5] > 0)
  newcardShape << Form("CMS_zhinv%s_MVAWjetsStatBounding_%s  shape    -     -	  -     -     -     -	1.000\n",finalStateName,ECMsb.Data());
  newcardShape.close();
  }
