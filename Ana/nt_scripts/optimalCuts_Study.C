...
void optimalCuts_Study
(
 int     mH  	 = 29,
 Char_t xTitle[]="myX", Char_t yTitle[]="Fraction",
 int thePlot = 0,
 TString bgdInputFile    = "ntuples_42x/backgroundA_skim2.root",
 TString signalInputFile = "ntuples_42x/hww160.root",
 TString dataInputFile   = "ntuples_42x/data_2l_skim2.root",
 ...
 )
{
  ...
  SmurfTree sigEvent;
  sigEvent.LoadTree(signalInputFile,0);
  sigEvent.InitTree(0);
  ...
  else if(thePlot >= 61 && thePlot <= 62) {nBinPlot = 100; xminPlot = 0.0; xmaxPlot =  1000.0;}
  nBin = nBinPlot;
  ...
  for (int i=0; i<nBgd; ++i) {
    ...
    //if(bgdEvent.dstype_ == SmurfTree::data && bgdEvent.run_ <  170826) continue;
    if(bgdEvent.dstype_ == SmurfTree::data && bgdEvent.run_ > 172802) continue;
    int fDecay = 0;
	...
	(bgdEvent.jet1_.Pt() <= 15. || bgdEvent.dPhiDiLepJet1_*180.0/TMath::Pi() < 165. || 
	 ...
	 (bgdEvent.type_ == lDecay || lDecay == 4 || (lDecay == 5 && (bgdEvent.type_ == SmurfTree::em || bgdEvent.type_ == SmurfTree::me))) &&
	 1 == 1
	){
	passCuts = true;
   ...
   //// bgdEvent.jet1Btag_ > 3.3 &&
   //// bgdEvent.jet2Btag_ > 3.3 &&
   //// bgdEvent.jet1_.Pt() > 30 &&
   //// bgdEvent.jet2_.Pt() > 20 &&
   ////(fabs(bgdEvent.dilep_.M()-91.1876) < 15.) && 
   ////(fabs(bgdEvent.dilep_.M()-91.1876) < 30.) && 
   //// bgdEvent.met_ < 40 &&
	 1 == 1
	){
	passCuts = true;
	...
	//  printf("PROBLEMC(%d): %f %f %f = %f - %f %f %f %f %f = %f\n",bgdEvent.event_,add1,add2,trigEff,add,bgdEvent.sfWeightFR_,bgdEvent.sfWeightPU_,bgdEvent.sfWeightEff_,bgdEvent.sfWeightTrig_,bgdEvent.sfWeightHPt_,bgdEvent.sfWeightFR_*bgdEvent.sfWeightPU_*bgdEvent.sfWeightEff_*bgdEvent.sfWeightTrig_*bgdEvent.sfWeightHPt_);
	  ...
	  if((fDecay == 29 || fDecay == 30 || fDecay == 14) &&
	     ...
	     else                     add=add*wwScaleFactor1jCut[binc]; 
          }
  	...
  	if(channel == 6){
  	  if(fDecay == 9) {
  	    if     (bgdEvent.njets_ == 0) add=add*1.000;
  	    else if(bgdEvent.njets_ == 1) add=add*1.000;
  	    else if(bgdEvent.njets_ >= 2) add=add*2.365;
  	  }
  	  if((fDecay == 5 || fDecay == 13)) {
  	    if     (bgdEvent.njets_ == 0) add=add*1.000;
  	    else if(bgdEvent.njets_ == 1) add=add*1.000; 
  	    else if(bgdEvent.njets_ >= 2) add=add*1.410;
  	  }
        }
	...
	theWeight              = bgdEvent.scale1fb_*lumi*add;
      ...
      else if(thePlot ==62) myVar = bgdEvent.higgsPt_;
  ...
  if((channel >= 0 && channel <= 8000)){
	...
	(sigEvent.jet1_.Pt() <= 15. || sigEvent.dPhiDiLepJet1_*180.0/TMath::Pi() < 165. || 
	 ...
	 (sigEvent.type_ == lDecay || lDecay == 4 || (lDecay == 5 && (sigEvent.type_ == SmurfTree::em || sigEvent.type_ == SmurfTree::me))) &&
	 1 == 1
	){
	passCuts = true;
      }
    ...
    if(passCuts == true){
      double add = 1.;
      double addPU = 1.;
      //addPU = nVtxScaleFactor(fhDNvtx,sigEvent.nvtx_);
	...
	//add = add * enhancementFactor(channel);
      ...
      else if(thePlot ==62) myVar = sigEvent.higgsPt_;
  ...
  for (int i=0; i<nData; ++i) {
    ...
    //if(dataEvent.run_ <  170826) continue;
    if(dataEvent.run_ >  172802) continue;
	...
	(dataEvent.jet1_.Pt() <= 15. || dataEvent.dPhiDiLepJet1_*180.0/TMath::Pi() < 165. || 
	 ...
	 (dataEvent.type_ == lDecay || lDecay == 4 || (lDecay == 5 && (dataEvent.type_ == SmurfTree::em || dataEvent.type_ == SmurfTree::me))) &&
	 1 == 1
	){
	passCuts = true;
   ...
   //// dataEvent.jet1Btag_ > 3.3 &&
   //// dataEvent.jet2Btag_ > 3.3 &&
   //// dataEvent.jet1_.Pt() > 30 &&
   //// dataEvent.jet2_.Pt() > 20 &&
   ////(fabs(dataEvent.dilep_.M()-91.1876) < 15.) && 
   ////(fabs(dataEvent.dilep_.M()-91.1876) < 30.) && 
   //// dataEvent.met_ < 40 &&
	 1 == 1
	){
	passCuts = true;
    ...
    if(passCuts == true){
      ...
      else if(thePlot ==62) myVar = dataEvent.higgsPt_;
  ...
  double bgdCombined[4] = {bgdDecay[0]+bgdDecay[1]+bgdDecay[2]+bgdDecay[3],
                           bgdDecay[6]+bgdDecay[7]+bgdDecay[8]+bgdDecay[9]+bgdDecay[31],
