...
// dataEstimations
//------------------------------------------------------------------------------
void dataEstimations
(
 int     mH  	 = 5,
 double mhAna = 4,
 TString bgdInputFile    = "ntuples_42x/backgroundA_skim0.root",
 TString signalInputFile = "ntuples_42x/hww160.root",
 TString dataInputFile   = "ntuples_42x/data_2l_skim0.root"
 )
{
  ...
  double btag_lowpt_1j_den_error[50],btag_lowpt_1j_num_error[50],btag_lowpt_0j_den_error[50],btag_lowpt_0j_num_error[50];
  double btag_highestpt_2j_den_error[50],btag_highestpt_2j_num_error[50],btag_highestpt_1j_den_error[50],btag_highestpt_1j_num_error[50];
  ...
  for(int i=0; i<50; i++){
    ...
    btag_lowpt_1j_den_error[i] = 0.0;
    btag_lowpt_1j_num_error[i] = 0.0;
    btag_lowpt_0j_den_error[i] = 0.0;
    btag_lowpt_0j_num_error[i] = 0.0;
    btag_highestpt_2j_den_error[i] = 0.0;
    btag_highestpt_2j_num_error[i] = 0.0;
    btag_highestpt_1j_den_error[i] = 0.0;
    btag_highestpt_1j_num_error[i] = 0.0;
    zll_ee[i] = 0.0;
    zll_mm[i] = 0.0;
  }
  ...
  for (int i=0; i<nBgd; ++i) {
    ...
    //if(bgdEvent.dstype_ == SmurfTree::data && bgdEvent.run_ >= 170826) continue;
    //if(bgdEvent.dstype_ == SmurfTree::data && bgdEvent.run_ <  170826) continue;
    ...
    if(nFake > 1){
      ...
      if(bgdEvent.dstype_ == SmurfTree::data){
        ...
        //if(TMath::Abs((bgdEvent.sfWeightFR_*bgdEvent.sfWeightPU_*bgdEvent.sfWeightEff_*bgdEvent.sfWeightTrig_*bgdEvent.sfWeightHPt_)-add)/add>0.0001)
	...
	theWeight	       = add;
        ...
        add = add*fakeRate(bgdEvent.lep2_.Pt(), bgdEvent.lep2_.Eta(), fhDFRMu, fhDFREl, (bgdEvent.cuts_ & SmurfTree::Lep2LooseMuV2)  == SmurfTree::Lep2LooseMuV2  && (bgdEvent.cuts_ & SmurfTree::Lep2FullSelection) != SmurfTree::Lep2FullSelection,
											(bgdEvent.cuts_ & SmurfTree::Lep2LooseEleV4) == SmurfTree::Lep2LooseEleV4 && (bgdEvent.cuts_ & SmurfTree::Lep2FullSelection) != SmurfTree::Lep2FullSelection);
	//add = add*nVtxScaleFactor(fhDNvtx,bgdEvent.nvtx_);
	...
	theWeight	       = -1.0 * bgdEvent.scale1fb_*lumi*add;
      ...
      //double add1 = nVtxScaleFactor(fhDNvtx,bgdEvent.nvtx_);
      double add1 = nPUScaleFactor(fhDPUS4,bgdEvent.npu_);
      //double add1 = scaleFactor(bgdEvent.lep1_.Pt(), bgdEvent.lep1_.Eta(), bgdEvent.lep2_.Pt(), bgdEvent.lep2_.Eta(), bgdEvent.nvtx_, 2);
      ...
      add = add1*add2*add3;
     // if(TMath::Abs((bgdEvent.sfWeightFR_*bgdEvent.sfWeightPU_*bgdEvent.sfWeightEff_*bgdEvent.sfWeightTrig_*bgdEvent.sfWeightHPt_)-add)/add>0.0001){
     //   printf("PROBLEMC: %f - %f %f %f %f %f = %f\n",add,bgdEvent.sfWeightFR_,bgdEvent.sfWeightPU_,bgdEvent.sfWeightEff_,bgdEvent.sfWeightTrig_,bgdEvent.sfWeightHPt_,bgdEvent.sfWeightFR_*bgdEvent.sfWeightPU_*bgdEvent.sfWeightEff_*bgdEvent.sfWeightTrig_*bgdEvent.sfWeightHPt_);
     //   printf("		     %f %f %f %f %f\n",1.0,add1,add2,add3,1.0);
     // }
      if(channel == 5){
  	if(fDecay == 9 && (bgdEvent.type_ == SmurfTree::mm || bgdEvent.type_ == SmurfTree::ee)) {
  	  if(bgdEvent.njets_ == 0) add=add*3.02;
  	  if(bgdEvent.njets_ == 1) add=add*2.81;
  	  if(bgdEvent.njets_ >= 2) add=add*4.84;
  	}
      }
      if(channel == 6 || (channel >= 100 && channel <= 800) || channel == 900 || channel == 901){
        if((fDecay == 5 ||fDecay == 13)) {
          if(bgdEvent.njets_ == 0) add=add*1.164;
          if(bgdEvent.njets_ == 1) add=add*1.121; 
          if(bgdEvent.njets_ >= 2) add=add*1.188; 
        }
      }
      if(fDecay == 3) {
        add=add*1.95; 
	...
	if(bgdEvent.jet1Btag_ >= 2.1 && bgdEvent.njets_ == 1){
	  ...
	  btag_lowpt_1j_den_error[classType+10*4]	       += theWeight*theWeight;
	  btag_lowpt_1j_den_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  ...
	  //if(bgdEvent.nSoftMuons_ > 0 || bgdEvent.jetLowBtag_ >= 2.1){
	    ...
	    btag_lowpt_1j_num_error[classType+10*4]		 += theWeight*theWeight;
	    btag_lowpt_1j_num_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  }
        }
	if(bgdEvent.njets_ == 0){
	  btag_lowpt_0j_den[classType+10*4]		 += theWeight;
	  btag_lowpt_0j_den[classType+10*bgdEvent.type_] += theWeight;
	  btag_lowpt_0j_den_error[classType+10*4]	       += theWeight*theWeight;
	  btag_lowpt_0j_den_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  if((bgdEvent.cuts_ & patternTopTagNotInJets) == patternTopTagNotInJets){
	    btag_lowpt_0j_num[classType+10*4]		   += theWeight;
	    btag_lowpt_0j_num[classType+10*bgdEvent.type_] += theWeight;
	    btag_lowpt_0j_num_error[classType+10*4]		 += theWeight*theWeight;
	    btag_lowpt_0j_num_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  }
        }
	if((bgdEvent.cuts_ & patternTopTagNotInJets) != patternTopTagNotInJets && bgdEvent.jet2Btag_ >= 2.1 && bgdEvent.njets_ == 2){
	  ...
	  btag_highestpt_2j_den_error[classType+10*4] 	           += theWeight*theWeight;
	  btag_highestpt_2j_den_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  if(bgdEvent.jet1Btag_ >= 2.1){
	    ...
	    btag_highestpt_2j_num_error[classType+10*4]	             += theWeight*theWeight;
	    btag_highestpt_2j_num_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  }
        }
	if((bgdEvent.cuts_ & patternTopTagNotInJets) != patternTopTagNotInJets && bgdEvent.njets_ == 1){
	  ...
	  btag_highestpt_1j_den_error[classType+10*4] 	           += theWeight*theWeight;
	  btag_highestpt_1j_den_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  if(bgdEvent.jet1Btag_ >= 2.1){
	    btag_highestpt_1j_num[classType+10*4]	       += theWeight;
	    btag_highestpt_1j_num[classType+10*bgdEvent.type_] += theWeight;
	    btag_highestpt_1j_num_error[classType+10*4]	             += theWeight*theWeight;
	    btag_highestpt_1j_num_error[classType+10*bgdEvent.type_] += theWeight*theWeight;
	  }
        }
      }
    } // btag study
  ...
  for (int i=0; i<nData; ++i) {
    ...
    //if(dataEvent.run_ >= 170826) continue;
    //if(dataEvent.run_ <  170826) continue;
	...
	if(dataEvent.jet1Btag_ >= 2.1 && dataEvent.njets_ == 1){
	  ...
	  btag_lowpt_1j_den_error[classType+10*4]	        += theWeight*theWeight;
	  btag_lowpt_1j_den_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  if((dataEvent.cuts_ & patternTopTagNotInJets) == patternTopTagNotInJets){
	  //if(dataEvent.nSoftMuons_ > 0 || dataEvent.jetLowBtag_ >= 2.1){
	    btag_lowpt_1j_num[classType+10*4]		    += theWeight;
	    btag_lowpt_1j_num[classType+10*dataEvent.type_] += theWeight;
	    btag_lowpt_1j_num_error[classType+10*4]		  += theWeight*theWeight;
	    btag_lowpt_1j_num_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  }
        }
	if(dataEvent.njets_ == 0){
	  btag_lowpt_0j_den[classType+10*4]		  += theWeight;
	  ...
	  btag_lowpt_0j_den_error[classType+10*4]	        += theWeight*theWeight;
	  btag_lowpt_0j_den_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  if((dataEvent.cuts_ & patternTopTagNotInJets) == patternTopTagNotInJets){
	    btag_lowpt_0j_num[classType+10*4]		    += theWeight;
	    btag_lowpt_0j_num[classType+10*dataEvent.type_] += theWeight;
	    btag_lowpt_0j_num_error[classType+10*4]		  += theWeight*theWeight;
	    btag_lowpt_0j_num_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  }
        }
	if((dataEvent.cuts_ & patternTopTagNotInJets) != patternTopTagNotInJets && dataEvent.jet2Btag_ >= 2.1 && dataEvent.njets_ == 2){
	  ...
	  btag_highestpt_2j_den_error[classType+10*4] 	            += theWeight*theWeight;
	  btag_highestpt_2j_den_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  if(dataEvent.jet1Btag_ >= 2.1){
	    btag_highestpt_2j_num[classType+10*4]	        += theWeight;
	    btag_highestpt_2j_num[classType+10*dataEvent.type_] += theWeight;
	    btag_highestpt_2j_num_error[classType+10*4]	              += theWeight*theWeight;
	    btag_highestpt_2j_num_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  }
        }
	if((dataEvent.cuts_ & patternTopTagNotInJets) != patternTopTagNotInJets && dataEvent.njets_ == 1){
	  ...
	  btag_highestpt_1j_den_error[classType+10*4] 	            += theWeight*theWeight;
	  btag_highestpt_1j_den_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  if(dataEvent.jet1Btag_ >= 2.1){
	    btag_highestpt_1j_num[classType+10*4]	        += theWeight;
	    btag_highestpt_1j_num[classType+10*dataEvent.type_] += theWeight;
	    btag_highestpt_1j_num_error[classType+10*4]	              += theWeight*theWeight;
	    btag_highestpt_1j_num_error[classType+10*dataEvent.type_] += theWeight*theWeight;
	  }
        }
      }
    } // btag study
  ...
  if(channel == 5){
    ...
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tt_2j[i] = (btag_highestpt_2j_num[1+i*10]                              )/(btag_highestpt_2j_den[1+i*10]                              );
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tw_2j[i] = (				    btag_highestpt_2j_num[2+i*10])/(                              btag_highestpt_2j_den[2+i*10]);
    for(int i=0; i<5; i++) effttMC_btag_highestpt_2j_error[i]    = sqrt((1.0-effttMC_btag_highestpt_2j[i])*effttMC_btag_highestpt_2j[i]/(btag_highestpt_2j_den[1+i*10]+btag_highestpt_2j_den[2+i*10])*
                                                                     (btag_highestpt_2j_den_error[1+i*10]+btag_highestpt_2j_den_error[2+i*10])/(btag_highestpt_2j_den[1+i*10]+btag_highestpt_2j_den[2+i*10]));
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tt_2j_error[i] = sqrt((1.0-effttMC_btag_highestpt_tt_2j[i])*effttMC_btag_highestpt_tt_2j[i]/(btag_highestpt_2j_den[1+i*10])*
                                                                     (btag_highestpt_2j_den_error[1+i*10])/(btag_highestpt_2j_den[1+i*10]));
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tw_2j_error[i] = sqrt((1.0-effttMC_btag_highestpt_tw_2j[i])*effttMC_btag_highestpt_tw_2j[i]/(btag_highestpt_2j_den[2+i*10])*
                                                                     (btag_highestpt_2j_den_error[2+i*10])/(btag_highestpt_2j_den[2+i*10]));

    for(int i=0; i<5; i++) effttDA_btag_highestpt_2j[i] = (btag_highestpt_2j_num[3+i*10]-btag_highestpt_2j_num[0+i*10]-btag_highestpt_2j_num[2+i*10]*btagSF)/
                                                          (btag_highestpt_2j_den[3+i*10]-btag_highestpt_2j_den[0+i*10]-btag_highestpt_2j_den[2+i*10]*btagSF);
    for(int i=0; i<5; i++) effttDA_btag_highestpt_2j_error[i] = sqrt((1-effttDA_btag_highestpt_2j[i])*effttDA_btag_highestpt_2j[i]/btag_highestpt_2j_den[3+i*10]);

    for(int i=0; i<5; i++) printf("numerator(%d)   --> data: %4.0f, background: %7.2f, tt+tw: %7.2f, tt: %7.2f, tw: %7.2f\n",i,btag_highestpt_2j_num[3+i*10],btag_highestpt_2j_num[0+i*10],(btag_highestpt_2j_num[1+i*10]+btag_highestpt_2j_num[2+i*10]),btag_highestpt_2j_num[1+i*10],btag_highestpt_2j_num[2+i*10]);
    for(int i=0; i<5; i++) printf("denominator(%d) --> data: %4.0f, background: %7.2f, tt+tw: %7.2f, tt: %7.2f, tw: %7.2f\n",i,btag_highestpt_2j_den[3+i*10],btag_highestpt_2j_den[0+i*10],(btag_highestpt_2j_den[1+i*10]+btag_highestpt_2j_den[2+i*10]),btag_highestpt_2j_den[1+i*10],btag_highestpt_2j_den[2+i*10]);
                           printf("channel eff_tttw eff_tttw_err eff_tt eff_tt_err eff_tw eff_tw_err eff_data eff_data_err\n");
    for(int i=0; i<5; i++) printf("eff (%d): %6.3f +/- %6.3f  %6.3f +/- %6.3f  %6.3f +/- %6.3f --> %6.3f +/- %6.3f\n",i,
    ...
    for(int i=0; i<5; i++) printf("scaleFactor2j(%d) --> %6.3f +/- %6.3f\n",i,(btag_highestpt_2j_num[3+i*10]-btag_highestpt_2j_num[0+i*10])/(btag_highestpt_2j_num[1+i*10]+btag_highestpt_2j_num[2+i*10]),
                                  sqrt(btag_highestpt_2j_num[3+i*10])/(btag_highestpt_2j_num[1+i*10]+btag_highestpt_2j_num[2+i*10]));
    ...
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tt_1j[i] = (btag_highestpt_1j_num[1+i*10]                              )/(btag_highestpt_1j_den[1+i*10]                              );
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tw_1j[i] = (                              btag_highestpt_1j_num[2+i*10])/(                              btag_highestpt_1j_den[2+i*10]);

    for(int i=0; i<5; i++) effttMC_btag_highestpt_1j_error[i] = sqrt((1.0-effttMC_btag_highestpt_1j[i])*effttMC_btag_highestpt_1j[i]/(btag_highestpt_1j_den[1+i*10]+btag_highestpt_1j_den[2+i*10])*
                                                                     (btag_highestpt_1j_den_error[1+i*10]+btag_highestpt_1j_den_error[2+i*10])/(btag_highestpt_1j_den[1+i*10]+btag_highestpt_1j_den[2+i*10]));
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tt_1j_error[i] = sqrt((1.0-effttMC_btag_highestpt_tt_1j[i])*effttMC_btag_highestpt_tt_1j[i]/(btag_highestpt_1j_den[1+i*10])*
                                                                     (btag_highestpt_1j_den_error[1+i*10])/(btag_highestpt_1j_den[1+i*10]));
    for(int i=0; i<5; i++) effttMC_btag_highestpt_tw_1j_error[i] = sqrt((1.0-effttMC_btag_highestpt_tw_1j[i])*effttMC_btag_highestpt_tw_1j[i]/(btag_highestpt_1j_den[2+i*10])*
                                                                     (btag_highestpt_1j_den_error[2+i*10])/(btag_highestpt_1j_den[2+i*10]));

    for(int i=0; i<5; i++) effttDA_btag_highestpt_1j[i] = (btag_highestpt_1j_num[3+i*10]-btag_highestpt_1j_num[0+i*10]-btag_highestpt_1j_num[2+i*10]*btagSF)/(btag_highestpt_1j_den[3+i*10]-btag_highestpt_1j_den[0+i*10]-btag_highestpt_1j_den[2+i*10]*btagSF);
    for(int i=0; i<5; i++) effttDA_btag_highestpt_1j_error[i] = sqrt((1-effttDA_btag_highestpt_1j[i])*effttDA_btag_highestpt_1j[i]/btag_highestpt_1j_den[3+i*10]);
    for(int i=0; i<5; i++) printf("numerator(%d)   --> data: %4.0f, background: %7.2f, tt+tw: %7.2f, tt: %7.2f, tw: %7.2f\n",i,btag_highestpt_1j_num[3+i*10],btag_highestpt_1j_num[0+i*10],(btag_highestpt_1j_num[1+i*10]+btag_highestpt_1j_num[2+i*10]),btag_highestpt_1j_num[1+i*10],btag_highestpt_1j_num[2+i*10]);
    for(int i=0; i<5; i++) printf("denominator(%d) --> data: %4.0f, background: %7.2f, tt+tw: %7.2f, tt: %7.2f, tw: %7.2f\n",i,btag_highestpt_1j_den[3+i*10],btag_highestpt_1j_den[0+i*10],(btag_highestpt_1j_den[1+i*10]+btag_highestpt_1j_den[2+i*10]),btag_highestpt_1j_den[1+i*10],btag_highestpt_1j_den[2+i*10]);
                           printf("channel eff_tttw eff_tttw_err eff_tt eff_tt_err eff_tw eff_tw_err eff_data eff_data_err\n");
    for(int i=0; i<5; i++) printf("eff (%d): %6.3f +/- %6.3f  %6.3f +/- %6.3f  %6.3f +/- %6.3f --> %6.3f +/- %6.3f  (indicative)\n",i,
    ...
    double estimationMC_btag_highestpt_1j_err[5]; for(int i=0; i<5; i++) estimationMC_btag_highestpt_1j_err[i] = effttMC_btag_highestpt_tt_1j_error[i]/effttMC_btag_highestpt_2j[4]/effttMC_btag_highestpt_2j[4]*(btag_highestpt_1j_num[1+i*10]+btag_highestpt_1j_num[2+i*10]);
    ...
    double estimationDA_btag_highestpt_1j_error[5]; for(int i=0; i<5; i++) estimationDA_btag_highestpt_1j_error[i] = sqrt(((btag_highestpt_1j_num[3+i*10]-btag_highestpt_1j_num[0+i*10])*effttDA_btag_highestpt_2j_error[4]/effttDA_btag_highestpt_2j[4]/effttDA_btag_highestpt_2j[4])*
                                                                                                                      ((btag_highestpt_1j_num[3+i*10]-btag_highestpt_1j_num[0+i*10])*effttDA_btag_highestpt_2j_error[4]/effttDA_btag_highestpt_2j[4]/effttDA_btag_highestpt_2j[4])+
    ...
    for(int i=0; i<5; i++) printf("top 1-jet(%d): %7.3f  %7.3f +/- %6.3f | %7.3f +/- %6.3f -SF-> %5.3f +/- %5.3f\n",i,(btag_highestpt_1j_den[1+i*10]+btag_highestpt_1j_den[2+i*10])-(btag_highestpt_1j_num[1+i*10]+btag_highestpt_1j_num[2+i*10]),
										   ...
										   estimationDA_btag_highestpt_1j[i]      /((btag_highestpt_1j_den[1+i*10]+btag_highestpt_1j_den[2+i*10])-(btag_highestpt_1j_num[1+i*10]+btag_highestpt_1j_num[2+i*10])),
										   estimationDA_btag_highestpt_1j_error[i]/((btag_highestpt_1j_den[1+i*10]+btag_highestpt_1j_den[2+i*10])-(btag_highestpt_1j_num[1+i*10]+btag_highestpt_1j_num[2+i*10])));

    printf("**********eff low pt jet 1-j**********\n");
    double btagSFTW = estimationDA_btag_highestpt_1j[4]      /((btag_highestpt_1j_den[1+4*10]+btag_highestpt_1j_den[2+4*10])-(btag_highestpt_1j_num[1+4*10]+btag_highestpt_1j_num[2+4*10]));
    printf("btagSFTW = %f\n",btagSFTW);
    ...
    for(int i=0; i<5; i++) effttMC_btag_lowpt_tt_1j[i] = (btag_lowpt_1j_num[1+10*i]                          )/(btag_lowpt_1j_den[1+10*i]                          );
    for(int i=0; i<5; i++) effttMC_btag_lowpt_tw_1j[i] = (                          btag_lowpt_1j_num[2+10*i])/(                          btag_lowpt_1j_den[2+10*i]);

    for(int i=0; i<5; i++) effttMC_btag_lowpt_1j_error[i] = sqrt((1.0-effttMC_btag_lowpt_1j[i])*effttMC_btag_lowpt_1j[i]/(btag_lowpt_1j_den[1+i*10]+btag_lowpt_1j_den[2+i*10])*
                                                                 (btag_lowpt_1j_den_error[1+i*10]+btag_lowpt_1j_den_error[2+i*10])/(btag_lowpt_1j_den[1+i*10]+btag_lowpt_1j_den[2+i*10]));
    for(int i=0; i<5; i++) effttMC_btag_lowpt_tt_1j_error[i] = sqrt((1.0-effttMC_btag_lowpt_tt_1j[i])*effttMC_btag_lowpt_tt_1j[i]/(btag_lowpt_1j_den[1+i*10])*
                                                                    (btag_lowpt_1j_den_error[1+i*10])/(btag_lowpt_1j_den[1+i*10]));
    for(int i=0; i<5; i++) effttMC_btag_lowpt_tw_1j_error[i] = sqrt((1.0-effttMC_btag_lowpt_tw_1j[i])*effttMC_btag_lowpt_tw_1j[i]/(btag_lowpt_1j_den[2+i*10])*
                                                                    (btag_lowpt_1j_den_error[2+i*10])/(btag_lowpt_1j_den[2+i*10]));

    for(int i=0; i<5; i++) effttDA_btag_lowpt_1j[i]     = (btag_lowpt_1j_num[3+10*i]-btag_lowpt_1j_num[0+10*i]-btag_lowpt_1j_num[2+10*i]*btagSF*btagSFTW)/ (btag_lowpt_1j_den[3+10*i]-btag_lowpt_1j_den[0+10*i]-btag_lowpt_1j_den[2+10*i]*btagSF*btagSFTW);
    for(int i=0; i<5; i++) effttDA_btag_lowpt_1j_error[i] = sqrt((1-effttDA_btag_lowpt_1j[i])*effttDA_btag_lowpt_1j[i]/btag_lowpt_1j_den[3+10*i]);
    for(int i=0; i<5; i++) printf("numerator(%d)   --> data: %4.0f, background: %7.2f, tt+tw: %7.2f, tt: %7.2f, tw: %7.2f\n",i,btag_lowpt_1j_num[3+i*10],btag_lowpt_1j_num[0+i*10],(btag_lowpt_1j_num[1+i*10]+btag_lowpt_1j_num[2+i*10]),btag_lowpt_1j_num[1+i*10],btag_lowpt_1j_num[2+i*10]);
    for(int i=0; i<5; i++) printf("denominator(%d) --> data: %4.0f, background: %7.2f, tt+tw: %7.2f, tt: %7.2f, tw: %7.2f\n",i,btag_lowpt_1j_den[3+i*10],btag_lowpt_1j_den[0+i*10],(btag_lowpt_1j_den[1+i*10]+btag_lowpt_1j_den[2+i*10]),btag_lowpt_1j_den[1+i*10],btag_lowpt_1j_den[2+i*10]);
                           printf("channel eff_tttw eff_tttw_err eff_tt eff_tt_err eff_tw eff_tw_err eff_data eff_data_err\n");
    for(int i=0; i<5; i++) printf("eff (%d): %6.3f +/- %6.3f  %6.3f +/- %6.3f  %6.3f +/- %6.3f --> %6.3f +/- %6.3f\n",i,
    ...
    double fttbar[5]; for(int i=0; i<5; i++) fttbar[i] = (btag_lowpt_0j_den[1+10*i]+btag_lowpt_0j_den[2+10*i]*ftw_b[i])/(btag_lowpt_0j_den[1+10*i]+btag_lowpt_0j_den[2+10*i]);
   
    double effMC_btag_lowpt_tt_0j_expected[5];  for(int i=0; i<5; i++) effMC_btag_lowpt_tt_0j_expected[i] = btag_lowpt_0j_num[1+10*i]/btag_lowpt_0j_den[1+10*i];
    double effMC_btag_lowpt_tw_0j_expected[5];  for(int i=0; i<5; i++) effMC_btag_lowpt_tw_0j_expected[i] = btag_lowpt_0j_num[2+10*i]/btag_lowpt_0j_den[2+10*i];
    double effMC_btag_lowpt_tt_0j[5];           for(int i=0; i<5; i++) effMC_btag_lowpt_tt_0j[i]          = 1-(1-effttMC_btag_lowpt_tt_1j[i])*(1-effttMC_btag_lowpt_tt_1j[i]);
    double effMC_btag_lowpt_tw_0j[5];           for(int i=0; i<5; i++) effMC_btag_lowpt_tw_0j[i]          = effttMC_btag_lowpt_tt_1j[i];
    for(int i=0; i<5; i++) printf("MC(%d), tt expected/measured: %5.3f/%5.3f, tw expected/measured: %5.3f/%5.3f\n",i,effMC_btag_lowpt_tt_0j_expected[i],effMC_btag_lowpt_tt_0j[i],effMC_btag_lowpt_tw_0j_expected[i],effMC_btag_lowpt_tw_0j[i]);
    ...
    //   fttbar[i] = (btag_lowpt_0j_den[1+10*i]+btag_lowpt_0j_den[2+10*i]*ftw_2b[i])/(btag_lowpt_0j_den[1+10*i]+btag_lowpt_0j_den[2+10*i]);
    //   printf("fttbar(%d) = %5.3f | ",i,fttbar[i]);
    // }
    // printf("\n");
    // end get closure test closing!!!!!!!!!!!!!!!

    double effMC_btag_lowpt_0j[5]; for(int i=0; i<5; i++) effMC_btag_lowpt_0j[i] = fttbar[i]*effMC_btag_lowpt_tt_0j[i]+(1-fttbar[i])*effMC_btag_lowpt_tw_0j[i];
    ...
    for(int i=0; i<5; i++) printf("fttbar= %5.3f, eff btag lowpt 0j(%d): %6.3f, %6.3f +/- %6.3f --> %6.3f +/- %6.3f\n",fttbar[i],i
                                                                                                       ...
                                                                                                       ,effMC_btag_lowpt_0j[i],effMC_btag_lowpt_0j_error[i],effDA_btag_lowpt_0j[i],effDA_btag_lowpt_0j_error[i]);
    ...
    double estimationMC_btag_lowpt_0j_error[5]; for(int i=0; i<5; i++) estimationMC_btag_lowpt_0j_error[i] = sqrt(((btag_lowpt_0j_num[1+i*10]+btag_lowpt_0j_num[2+i*10])*effMC_btag_lowpt_0j_error[4]/effMC_btag_lowpt_0j[4]/effMC_btag_lowpt_0j[4])*
                                                                                                              ((btag_lowpt_0j_num[1+i*10]+btag_lowpt_0j_num[2+i*10])*effMC_btag_lowpt_0j_error[4]/effMC_btag_lowpt_0j[4]/effMC_btag_lowpt_0j[4]));
    double estimationDA_btag_lowpt_0j_error[5]; for(int i=0; i<5; i++) estimationDA_btag_lowpt_0j_error[i] = sqrt(((btag_lowpt_0j_num[3+i*10]-btag_lowpt_0j_num[0+i*10])*effDA_btag_lowpt_0j_error[4]/effDA_btag_lowpt_0j[4]/effDA_btag_lowpt_0j[4])*
                                                                                                              ((btag_lowpt_0j_num[3+i*10]-btag_lowpt_0j_num[0+i*10])*effDA_btag_lowpt_0j_error[4]/effDA_btag_lowpt_0j[4]/effDA_btag_lowpt_0j[4])+
