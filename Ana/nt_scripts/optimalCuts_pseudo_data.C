...
#include "/home/ceballos/releases/CMSSW_5_2_3_patch3/src/Smurf/Analysis/HWWlvlv/OtherBkgScaleFactors.h"
#include "/home/ceballos/releases/CMSSW_5_2_3_patch3/src/Smurf/Analysis/HWWlvlv/DYBkgScaleFactors.h"
#include "/home/ceballos/releases/CMSSW_5_2_3_patch3/src/Smurf/Analysis/HWWlvlv/TopBkgScaleFactors.h"
#include "/home/ceballos/releases/CMSSW_5_2_3_patch3/src/Smurf/Analysis/HWWlvlv/WWBkgScaleFactors.h"
#include "/home/ceballos/releases/CMSSW_5_2_3_patch3/src/Smurf/Analysis/HWWlvlv/HWWCuts.h"

const int verboseLevel =   1;

//------------------------------------------------------------------------------
// optimalCuts_pseudo_data
//------------------------------------------------------------------------------
// GF  == 10010, WBF == 10001, WH == 26, ZH == 24, ttH=121/122
void optimalCuts_pseudo_data
(
 int     mH  	 = 29,
 Char_t xTitle[]="myX", Char_t yTitle[]="Fraction",
 int thePlot = 7,
 TString bgdInputFile    = "ntuples_42x_All/backgroundC_skim1.root",
 TString signalInputFile = "ntuples_42x_All/hww160.root",
 TString dataInputFile   = "/data/smurf/ceballos/ntuples/pseudo-data_4700ipb/pseudo_data_11.root",
 ...
 )
{
  ...
  else if(thePlot >= 63 && thePlot <= 63) {nBinPlot = 40; xminPlot = -0.5; xmaxPlot =  39.5;}
  nBin = nBinPlot;
  ...
  for (int i=0; i<nBgd; ++i) {
    ...
    if(passCuts == true){
	  ...
	  fDecay                 = 3;
	  isSignalDecay          = false;
	  theWeight              = -1.0 * bgdEvent.scale1fb_*lumi*add*0.0;
          bgdDecay[(int)fDecay] += theWeight;
	  ...
	  theWeight = 0.0;
        ...
        theWeight = ZttScaleFactor(bgdEvent.nvtx_,period,bgdEvent.scale1fb_)*lumi*0.0;
        bgdDecay[(int)fDecay] += theWeight;
        weiDecay[(int)fDecay] += TMath::Abs(theWeight)*TMath::Abs(theWeight);
      }
	...
	//double myWeight = bgdEvent.scale1fb_*lumi*(bgdEvent.sfWeightFR_*bgdEvent.sfWeightPU_*bgdEvent.sfWeightEff_*bgdEvent.sfWeightTrig_*bgdEvent.sfWeightHPt_);
	  ...
	  printf("PROBLEMC(%d): %f %f %f = %f - %f %f %f %f %f = %f\n",bgdEvent.event_,add1,add2,trigEff,add,bgdEvent.sfWeightFR_,bgdEvent.sfWeightPU_,bgdEvent.sfWeightEff_,bgdEvent.sfWeightTrig_,bgdEvent.sfWeightHPt_,bgdEvent.sfWeightFR_*bgdEvent.sfWeightPU_*bgdEvent.sfWeightEff_*bgdEvent.sfWeightTrig_*bgdEvent.sfWeightHPt_);
        add = 1.0;
	if(bgdEvent.dstype_ == SmurfTree::wgstar) add = 0.0;
	if(bgdEvent.dstype_ == SmurfTree::wgamma) add = 0.0;
	if(fDecay == 9) 			  add = 0.0;

	// CAREFUL HERE, no data-driven corrections
	// add = 1.0;
	double special_factor = 0.907;
	if     (bgdEvent.njets_ == 1) special_factor = 0.842;
	else if(bgdEvent.njets_ >= 2) special_factor = 0.860;
	theWeight              = bgdEvent.scale1fb_*lumi*add*scaleFactor(bgdEvent.lep1_.Pt(), bgdEvent.lep1_.Eta(), bgdEvent.lep2_.Pt(), bgdEvent.lep2_.Eta(), bgdEvent.nvtx_, 2)*special_factor;
        bgdDecay[(int)fDecay] += theWeight;
        weiDecay[(int)fDecay] += theWeight*theWeight;
      }
      double myVar = newMet;
      ...
      else if(thePlot ==63) myVar = bgdEvent.npu_;
  ...
  if((channel >= 0 && channel <= 8000)){
    ...
    if(nJetsType == 2){ // nJetsType = 0/1/2-jet selection
      ...
      if(TMath::Abs(sigEvent.jet1_.eta()) >= 4.5 || TMath::Abs(sigEvent.jet2_.eta()) >= 4.5)                 Njet3 = 0;
    }
    int centrality = 0;
    ...
    if(sigEvent.dilep_.Pt() <= 45) passNewCuts = false;
    ...
    if(passCuts == true){
      double add = 1.;
      double addPU = 1.;
      addPU = nPUScaleFactor(fhDPUS4,sigEvent.npu_);
      //add = add*scaleFactor(bgdEvent.lep1_.Pt(), bgdEvent.lep1_.Eta(), bgdEvent.lep2_.Pt(), bgdEvent.lep2_.Eta(), bgdEvent.nvtx_, 2);
      ...
      add = scaleFactor(sigEvent.lep1_.Pt(), sigEvent.lep1_.Eta(), sigEvent.lep2_.Pt(), sigEvent.lep2_.Eta(), sigEvent.nvtx_, 2)*0.89;
      // This is tricky, we have Fall11 samples only
      if(mH == 118 || mH == 122 || mH == 124 || mH == 126 || mH == 128 || mH == 135) {
        add = sigEvent.sfWeightPU_*sigEvent.sfWeightEff_*sigEvent.sfWeightTrig_;
      }
      // CAREFUL HERE, no data-driven corrections, just Higgs k-factors

      if (sigEvent.processId_ == 10010) {
        float theMax = 0.00;
        add = add * HiggsPtKFactor->GetBinContent( HiggsPtKFactor->GetXaxis()->FindFixBin(TMath::Max(sigEvent.higgsPt_, theMax)));
	//add = add * enhancementFactor(channel);
      ...
      //if(TMath::Abs((sigEvent.sfWeightFR_*sigEvent.sfWeightPU_*sigEvent.sfWeightEff_*sigEvent.sfWeightTrig_*sigEvent.sfWeightHPt_)-add)/add>0.0001) {
      ...
      else if(thePlot ==63) myVar = sigEvent.npu_;
  ...
  for (int i=0; i<nData; ++i) {
    ...
    if(dataEvent.nvtx_ >  22) continue;
    ...
    if(nJetsType == 2){ // nJetsType = 0/1/2-jet selection
      ...
      if(TMath::Abs(dataEvent.jet1_.eta()) >= 4.5 || TMath::Abs(dataEvent.jet2_.eta()) >= 4.5)                   Njet3 = 0;
    }
    int centrality = 0;
    ...
    if(passCuts == true){
      ...
      else if(thePlot ==63) myVar = dataEvent.npu_;
