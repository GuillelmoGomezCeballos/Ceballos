// $Id: runHWWSelection_iso.C,v 1.7 2013/01/20 11:45:28 ceballos Exp $
                     ...
                     int nsel = 620, int NEvents = 100)
{
  ...
  MuonIDMod *muonIDNoIso = new MuonIDMod;
  muonIDNoIso->SetIntRadius(fIntRadius);
  muonIDNoIso->SetClassType("GlobalTracker");
  muonIDNoIso->SetIDType(theWWMuId);
  muonIDNoIso->SetIsoType("NoIso");
  muonIDNoIso->SetApplyD0Cut(kTRUE);
  muonIDNoIso->SetApplyDZCut(kTRUE);
  muonIDNoIso->SetWhichVertex(0);
  muonIDNoIso->SetRhoType(theRhoType);
  muonIDNoIso->SetCleanMuonsName("CleanMuonsNoIso");
  muonIDNoIso->SetPtMin(1.0);
  ...
  muonID1->SetIsoType("PFIsoNoL");
  muonID1->SetApplyD0Cut(kTRUE);
  muonID1->SetApplyDZCut(kTRUE);
  muonID1->SetWhichVertex(0);
  muonID1->SetRhoType(theRhoType);
  muonID1->SetInputName(muonIDNoIso->GetOutputName());
  muonID1->SetOldMuonsName(muonIDNoIso->GetOutputName());
  muonID1->SetOldElectronsName(electronID1->GetOutputName());
  muonID1->SetPtMin(1.0);
  ...
  separatePileUpMod1->Add(muonIDNoIso);
  muonIDNoIso->Add(electronID1);
  electronID1->Add(muonID1);
  muonID1->Add(pubJet1);
  pubJet1->Add(jetCorr1_ntuple);
  jetCorr1_ntuple->Add(electronCleaning1);
  electronCleaning1->Add(photonIDMod1);
  photonIDMod1->Add(photonCleaningMod1);
  photonCleaningMod1->Add(theJetID2_ntuple);
  ...
  hltmod->Add(WWEvtSelMod1);
  WWEvtSelMod1->Add(HwwMakeNtupleMod0);
